...
from metric.similarity_metrics import SimilarityMetric
from sklearn import tree
from sklearn.ensemble import RandomForestClassifier
from data_compressor.compressor import Compressor
from data_compressor.compressors_provider import CompressorsProvider
import random
...
class MlMethodSelector:
  ...
  def train(self, use_random_forest = False):
    ...
    # if use_random_forest:
    #   depth_arr = [estimator.tree_.max_depth for estimator in self.classifier.estimators_]
    #   depth_avg = sum(depth_arr) / len(depth_arr)
    #   node_arr = [estimator.tree_.node_count for estimator in self.classifier.estimators_]
    #   node_avg = sum(node_arr) / len(node_arr)
    #   print("RF:", depth_avg, len(depth_arr), node_avg)
    # else:
    #   print("DT:", self.classifier.tree_.max_depth, self.classifier.tree_.node_count)
